        ...
        public fa_Dashboard_Info FacturadoPorDia(int IdEmpresa)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(ConexionesERP.GetConnectionString()))
                {
                    ...
                    + " select @TotalCobro = SUM(B.dc_ValorPago) from cxc_cobro as a with(nolock)"
                    + " join cxc_cobro_det as b with(nolock) on a.IdEmpresa = b.IdEmpresa and a.IdSucursal = b.IdSucursal and a.IdCobro = b.IdCobro"
                    ...
                    + " group by a.IdEmpresa"
                    + " select @TotalPagos = sum(a.cb_Valor)  from ba_Cbte_Ban as a with(nolock) "
