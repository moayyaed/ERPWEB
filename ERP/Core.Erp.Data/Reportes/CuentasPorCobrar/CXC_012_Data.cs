    ...
    public class CXC_012_Data
    {
        public List<CXC_012_Info> get_list(int IdEmpresa, int IdSucursal, decimal IdCliente, int Idtipo_cliente, DateTime fechaCorte, bool MostrarSoloCarteraVencida, int DiasVencimiento)
        {
            try
            {
                ...
                decimal IdTipoClienteIni = Idtipo_cliente;
                decimal IdTipoClienteFin = Idtipo_cliente == 0 ? 9999999 : Idtipo_cliente;

                fechaCorte = fechaCorte.Date;

                List<CXC_012_Info> Lista;
                using (Entities_reportes Context = new Entities_reportes())
                {
                    Lista = (from q in Context.SPCXC_012(IdEmpresa, IdSucursalIni, IdSucursalFin, IdClienteIni, IdClienteFin, IdTipoClienteIni, IdTipoClienteFin, fechaCorte, MostrarSoloCarteraVencida, DiasVencimiento)
                             select new CXC_012_Info
                             {
                                 ...
                                 NombreProbabilidad = q.NombreProbabilidad
                             }).ToList();
                }
                return Lista;
            }
