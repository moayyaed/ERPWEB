...
namespace Core.Erp.Web.Areas.Compras.Controllers
{
    public class OrdenServicioController : Controller
    {
        ...
        com_OrdenServicio_local_List Lista_OrdenServicio = new com_OrdenServicio_local_List();
        ...
        {
            List<in_Producto_Info> Lista = bus_producto.get_list_bajo_demanda(args, Convert.ToInt32(SessionFixed.IdEmpresa), cl_enumeradores.eTipoBusquedaProducto.SOLOSERVICIOS, cl_enumeradores.eModulo.FAC, Convert.ToInt32(SessionFixed.IdSucursal));
            return Lista;
        }
        ...
        public ActionResult CmbCentroCosto_OS()
        {
            string model = string.Empty;
            return PartialView("_CmbCentroCosto_OS", model);
        }
        ...
        public ActionResult Index()
        {
            ...
            var lst = bus_ordencompra.get_list(model.IdEmpresa, model.IdSucursal, model.fecha_ini, model.fecha_fin, true, model.IdTipoServicio);
            Lista_OrdenServicio.set_list(lst, model.IdTransaccionSession);
            return View(model);
        }
        [HttpPost]
        ...
        [ValidateInput(false)]
        public ActionResult GridViewPartial_orden_servicio(bool Nuevo=false)
        {
            ...
            var model = Lista_OrdenServicio.get_list(Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
        ...
        public ActionResult Consultar(int IdEmpresa = 0, int IdSucursal = 0, decimal IdOrdenCompra = 0, bool Exito = false)
        {
            #region Validar Session
            if (string.IsNullOrEmpty(SessionFixed.IdTransaccionSession))
                return RedirectToAction("Login", new { Area = "", Controller = "Account" });
            ...
            {
                //if (model.Estado=="A" && model.IdEstadoAprobacion_cat== "xAPRO")
                //{

                //}
                if (model.Estado == "A" && model.IdEstadoAprobacion_cat == "APRO")
                {
                    info.Modificar = false;
                }
            }
    ...
    public class com_OrdenServicio_local_List
    {
        string Variable = "com_OrdenServicio_local_Info";
        public List<com_ordencompra_local_Info> get_list(decimal IdTransaccionSession)
        {
            ...
            return (List<com_ordencompra_local_Info>)HttpContext.Current.Session[Variable + IdTransaccionSession];
        }
