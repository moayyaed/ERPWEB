...
namespace Core.Erp.Web.Areas.Facturacion.Controllers
{
    ...
    public class FacturaController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var lst = bus_factura.get_list(model.IdEmpresa, model.IdSucursal, model.fecha_ini, model.fecha_fin);
            Lista_Factura.set_list(lst, model.IdTransaccionSession);
            return View(model);
        }
        [HttpPost]
        ...
        [ValidateInput(false)]
        public ActionResult GridViewPartial_factura(bool Nuevo=false)
        {
                ...
                foreach (var item in array)
                {
                    ...
                    if (pf != null)
                        if (lst_det_f.Where(q => q.IdEmpresa_pf == pf.IdEmpresa_pf && q.IdSucursal_pf == pf.IdSucursal_pf && q.IdProforma == pf.IdProforma && q.Secuencia_pf == pf.Secuencia_pf).Count() == 0)
                        {
                            ...
                            List_det.AddRow(pf, IdTransaccionSession);
                            //lst_det_f.Add(pf);
                        }
                }
    ...
    public class fa_factura_List
    {
        string Variable = "fa_factura_consulta_Info";
        public List<fa_factura_consulta_Info> get_list(decimal IdTransaccionSession)
        {
            if (HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()] == null)
            {
                List<fa_factura_consulta_Info> list = new List<fa_factura_consulta_Info>();

                HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()] = list;
            }
            return (List<fa_factura_consulta_Info>)HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()];
        }

        public void set_list(List<fa_factura_consulta_Info> list, decimal IdTransaccionSession)
        {
            HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()] = list;
        }
    }
