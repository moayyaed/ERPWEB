...
SELECT OG.IdEmpresa, OG.IdCbteCble_Ogiro, OG.IdTipoCbte_Ogiro, OG.IdOrden_giro_Tipo, OG.IdProveedor, OG.co_serie, OG.co_factura + '/' + CAST(OG.IdCbteCble_Ogiro AS varchar(20)) AS co_factura, OG.co_FechaFactura, 
                  ...
                  dbo.cp_TipoDocumento.Codigo AS cod_Documento, OG.Estado, CASE WHEN conci.IdConciliacion_Caja IS NULL THEN cast(0 AS bit) ELSE cast(1 AS bit) END en_conci_caja, a.IdCuota, a.Secuencia, A.Valor_cuota, OG.IdTipoMovi
...
tb_persona AS per ON per.IdPersona = cp_proveedor.IdPersona LEFT JOIN
    (SELECT IdEmpresa_Ogiro, IdTipoCbte_Ogiro, IdCbteCble_Ogiro, sum(de.re_valor_retencion) re_valor_retencion
     ...
     GROUP BY IdEmpresa_Ogiro, IdTipoCbte_Ogiro, IdCbteCble_Ogiro) AS ret ON OG.IdEmpresa = ret.IdEmpresa_Ogiro AND og.IdTipoCbte_Ogiro = ret.IdTipoCbte_Ogiro AND og.IdCbteCble_Ogiro = ret.IdCbteCble_Ogiro
...
SELECT dbo.cp_nota_DebCre.IdEmpresa, dbo.cp_nota_DebCre.IdCbteCble_Nota, dbo.cp_nota_DebCre.IdTipoCbte_Nota, '05' AS Expr4, dbo.cp_nota_DebCre.IdProveedor, CASE WHEN cp_nota_DebCre.cn_serie1 IS NULL 
                  THEN '' ELSE isnull(cp_nota_DebCre.cn_serie1, '') + '-' + isnull(cp_nota_DebCre.cn_serie2, '') END AS serie, CASE WHEN cp_nota_DebCre.cod_nota IS NULL OR
                  cp_nota_DebCre.cod_nota = '' THEN CAST(cp_nota_DebCre.IdCbteCble_Nota AS varchar(20)) ELSE cp_nota_DebCre.cod_nota END AS cn_Nota, dbo.cp_nota_DebCre.cn_fecha, dbo.cp_nota_DebCre.cn_observacion, 
                  ...
                  dbo.ct_cbtecble_tipo.tc_TipoCbte, dbo.cp_orden_pago_tipo_x_empresa.IdTipo_op, dbo.cp_orden_pago_tipo_x_empresa.IdEstadoAprobacion, isnull(a.Fecha_vcto_cuota, dbo.cp_nota_DebCre.cn_Fecha_vcto) cn_Fecha_vcto, NULL 
                  IdTipoFlujo, '' AS nom_flujo, dbo.tb_persona.IdPersona, 'N/D' AS Expr6, dbo.cp_nota_DebCre.Estado, CASE WHEN conci.IdConciliacion_Caja IS NULL THEN cast(0 AS bit) ELSE cast(1 AS bit) END en_conci_caja, a.IdCuota, a.Secuencia, 
                  A.Valor_cuota, NULL
