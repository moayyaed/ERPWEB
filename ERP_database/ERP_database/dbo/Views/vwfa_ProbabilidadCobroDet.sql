CREATE VIEW  vwfa_ProbabilidadCobroDet
AS
SELECT fa_factura.IdEmpresa, fa_factura.IdSucursal, fa_factura.IdBodega, fa_factura.IdCbteVta, fa_factura.vt_tipoDoc, fa_factura.vt_serie1 + '-' + fa_factura.vt_serie2 + '-' + fa_factura.vt_NumFactura AS vt_NumFactura, fa_factura.vt_fecha, 
				  ...
				  case when datediff(day,fa_factura.vt_fech_venc,getdate())  < 0 then 0 else datediff(day,fa_factura.vt_fech_venc,getdate()) end DiasVencido,
				  fa_factura.vt_Observacion, fa_factura_resumen.SubtotalConDscto, fa_factura_resumen.ValorIVA, fa_factura_resumen.Total, tb_persona.pe_nombreCompleto, 
                  isnull(round(SUM(cxc_cobro_det.dc_ValorPago),2),0) AS dc_ValorPago, round(fa_factura_resumen.Total - isnull(SUM(cxc_cobro_det.dc_ValorPago),0),2) Saldo, fa_ProbabilidadCobroDet.IdProbabilidad
FROM     fa_factura INNER JOIN
				  ...
				  fa_ProbabilidadCobroDet ON fa_ProbabilidadCobroDet.IdEmpresa = fa_factura.IdEmpresa AND fa_ProbabilidadCobroDet.IdSucursal = fa_factura.IdSucursal
				  AND fa_ProbabilidadCobroDet.IdBodega = fa_factura.IdBodega AND fa_ProbabilidadCobroDet.IdCbteVta = fa_factura.IdCbteVta AND fa_ProbabilidadCobroDet.vt_tipoDoc = fa_factura.vt_tipoDoc 
...
SELECT fa_notaCreDeb.IdEmpresa, fa_notaCreDeb.IdSucursal, fa_notaCreDeb.IdBodega, fa_notaCreDeb.IdNota, fa_notaCreDeb.CodDocumentoTipo, 
		CASE WHEN fa_notaCreDeb.NaturalezaNota = 'SRI' THEN fa_notaCreDeb.Serie1+'-'+fa_notaCreDeb.Serie2+'-'+fa_notaCreDeb.NumNota_Impresa ELSE ISNULL(fa_notaCreDeb.CodDocumentoTipo,CAST(fa_notaCreDeb.IdNota AS VARCHAR(20))) END AS vt_NumFactura, fa_notaCreDeb.no_fecha, 
                  fa_notaCreDeb.no_fecha_venc, 
				  case when datediff(day,fa_notaCreDeb.no_fecha_venc,getdate())  < 0 then 0 else datediff(day,fa_notaCreDeb.no_fecha_venc,getdate()) end DiasVencido,
				  fa_notaCreDeb.sc_observacion, fa_notaCreDeb_resumen.SubtotalConDscto, fa_notaCreDeb_resumen.ValorIVA, fa_notaCreDeb_resumen.Total, tb_persona.pe_nombreCompleto, 
                  isnull(round(SUM(cxc_cobro_det.dc_ValorPago),2),0) AS dc_ValorPago, round(fa_notaCreDeb_resumen.Total - isnull(SUM(cxc_cobro_det.dc_ValorPago),0),2) Saldo, fa_ProbabilidadCobroDet.IdProbabilidad
FROM     fa_notaCreDeb INNER JOIN
                  ...
                  fa_notaCreDeb.IdNota = fa_notaCreDeb_resumen.IdNota INNER JOIN
				  ...
				  fa_ProbabilidadCobroDet ON fa_ProbabilidadCobroDet.IdEmpresa = fa_notaCreDeb.IdEmpresa AND fa_ProbabilidadCobroDet.IdSucursal = fa_notaCreDeb.IdSucursal
				  AND fa_ProbabilidadCobroDet.IdBodega = fa_notaCreDeb.IdBodega AND fa_ProbabilidadCobroDet.IdCbteVta = fa_notaCreDeb.IdNota AND fa_ProbabilidadCobroDet.vt_tipoDoc = fa_notaCreDeb.CodDocumentoTipo
GROUP BY fa_notaCreDeb.IdEmpresa, fa_notaCreDeb.IdSucursal, fa_notaCreDeb.IdBodega, fa_notaCreDeb.IdNota, fa_notaCreDeb.CodDocumentoTipo, fa_notaCreDeb.Serie1 + '-' + fa_notaCreDeb.Serie2 + '-' + fa_notaCreDeb.NumNota_Impresa, fa_notaCreDeb.no_fecha,fa_notaCreDeb.no_fecha_venc,  
                  fa_notaCreDeb.no_fecha_venc, fa_notaCreDeb.sc_observacion, fa_notaCreDeb_resumen.SubtotalConDscto, fa_notaCreDeb_resumen.ValorIVA, fa_notaCreDeb_resumen.Total, tb_persona.pe_nombreCompleto, fa_notaCreDeb.NaturalezaNota, fa_ProbabilidadCobroDet.IdProbabilidad
