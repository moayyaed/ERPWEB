...
AS
DELETE [web].[ct_CONTA_005] where IdUsuario = @IdUsuario

INSERT INTO [web].[ct_CONTA_005]
           ([IdEmpresa]
           ,[IdPunto_cargo]
           ,[IdUsuario]
           ,[IdPunto_cargo_grupo]
...
BEGIN --SALDO INICIAL
	UPDATE [web].[ct_CONTA_005] set SaldoAnterior = A.Valor
	from(
	...
	where [web].[ct_CONTA_005].IdEmpresa = a.IdEmpresa
	and [web].[ct_CONTA_005].IdPunto_cargo = a.IdPunto_cargo
	and [web].[ct_CONTA_005].IdUsuario = @IdUsuario
END

BEGIN --DEBITOS
	UPDATE [web].[ct_CONTA_005] set Debitos = A.Valor
	from(
	SELECT d.IdEmpresa, d.IdPunto_cargo, round(sum(d.dc_Valor),2) Valor
	FROM ct_cbtecble AS C INNER JOIN 
	ct_cbtecble_det AS D ON C.IdEmpresa = D.IdEmpresa AND C.IdTipoCbte = D.IdTipoCbte AND C.IdCbteCble = D.IdCbteCble
...
BEGIN --CREDITOS
	UPDATE [web].[ct_CONTA_005] set Creditos = ABS(A.Valor)
	from(
	SELECT d.IdEmpresa, d.IdPunto_cargo, round(sum(d.dc_Valor),2) Valor
	FROM ct_cbtecble AS C INNER JOIN 
	ct_cbtecble_det AS D ON C.IdEmpresa = D.IdEmpresa AND C.IdTipoCbte = D.IdTipoCbte AND C.IdCbteCble = D.IdCbteCble
...
BEGIN --SALDO FINAL
	UPDATE [web].[ct_CONTA_005] set SaldoFinal = A.Valor
	from(
	SELECT d.IdEmpresa, d.IdPunto_cargo, round(sum(d.dc_Valor),2) Valor
	FROM ct_cbtecble AS C INNER JOIN 
	ct_cbtecble_det AS D ON C.IdEmpresa = D.IdEmpresa AND C.IdTipoCbte = D.IdTipoCbte AND C.IdCbteCble = D.IdCbteCble
...
IF(@MostrarSaldo0 = 0)
begin
delete [web].[ct_CONTA_005] where IdUsuario = @IdUsuario and SaldoFinal = 0
end

select [IdEmpresa]           ,[IdPunto_cargo]           ,[IdUsuario]           ,[IdPunto_cargo_grupo]           ,[nom_punto_cargo]           ,[nom_punto_cargo_grupo]
           ,[SaldoAnterior]           ,[Debitos]           ,[Creditos]           ,[SaldoFinal] from [web].[ct_CONTA_005]
