--exec [web].[SPCXP_019] 1,'3/10/2020',1,9999,1,99999,2311,2311
CREATE PROCEDURE [web].[SPCXP_019]
  @idempresa int ,
  @fecha datetime ,
  @IdSucursalIni int,
  @IdSucursalFin int,
...
vw.IdEmpresa, vw.IdProveedor, vw.nom_proveedor, SUM(vw.Valor_a_pagar)Valor_a_pagar, SUM(MontoAplicado)MontoAplicado, SUM(Saldo)Saldo, Ruc_Proveedor, representante_legal,
SUM(x_Vencer)x_Vencer, sum(Vencido) Vencido,sum(Vencido_1_30)Vencido_1_30, sum(Vencido_31_60)Vencido_31_60, sum(Vencido_60_90)Vencido_60_90, sum(Vencido_mayor_90)Vencido_mayor_90, Su_Descripcion, IdClaseProveedor, descripcion_clas_prove, EsRelacionado
from(         
Select 
 ...
 (
			...
			left join (
					...
					) conci on og.IdEmpresa = conci.IdEmpresa_OGiro and og.IdTipoCbte_Ogiro = conci.IdTipoCbte_Ogiro and og.IdCbteCble_Ogiro = conci.IdCbteCble_Ogiro inner join
					cp_proveedor_clase as c on pro.IdEmpresa = c.IdEmpresa and pro.IdClaseProveedor = c.IdClaseProveedor
			WHERE og.Estado = 'A'
			...
			and og.IdOrdenPago = d.IdOrdenPago inner join tb_sucursal as su on og.IdEmpresa = su.IdEmpresa and og.IdSucursal = su.IdSucursal inner join cp_proveedor_clase as c
			on pro.IdEmpresa = c.IdEmpresa and pro.IdClaseProveedor = c.IdClaseProveedor
...
b on
 ...
 where vw.Saldo > 0
 group by vw.IdEmpresa, vw.IdProveedor, vw.nom_proveedor, Su_Descripcion, IdClaseProveedor, descripcion_clas_prove, EsRelacionado,Ruc_Proveedor, representante_legal
END