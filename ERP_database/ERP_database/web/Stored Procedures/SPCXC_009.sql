...
isnull(fa_notaCreDeb.Serie1 + '-' + fa_notaCreDeb.Serie2 + '-' + fa_notaCreDeb.NumNota_Impresa, fa_notaCreDeb.CodNota)AS NumDocumento, fa_notaCreDeb.no_fecha, fa_notaCreDeb.no_fecha_venc, 
                  tb_sucursal.Su_Descripcion, fa_notaCreDeb.IdCliente, tb_persona.pe_nombreCompleto, tb_persona.pe_cedulaRuc, 0, '' Ve_Vendedor, fa_notaCreDeb_resumen.Total, CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) < 0 THEN 0 ELSE DATEDIFF(DAY,fa_notaCreDeb.no_fecha_venc,@FechaCorte) END DiasVencido,
				  ...
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) < 0  THEN round(fa_notaCreDeb_resumen.Total - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS X_VENCER,
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) BETWEEN 1 AND 30  THEN round(fa_notaCreDeb_resumen.Total - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO30, 
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) BETWEEN 31 AND 60 THEN round(fa_notaCreDeb_resumen.Total - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO60,
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) BETWEEN 61 AND 90 THEN round(fa_notaCreDeb_resumen.Total - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO90,
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) >= 91 THEN round(fa_notaCreDeb_resumen.Total - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO90MAS
