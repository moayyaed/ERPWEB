...
isnull(fa_notaCreDeb.Serie1 + '-' + fa_notaCreDeb.Serie2 + '-' + fa_notaCreDeb.NumNota_Impresa, fa_notaCreDeb.CodNota)AS NumDocumento, fa_notaCreDeb.no_fecha, fa_notaCreDeb.no_fecha_venc, 
				  ...
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) BETWEEN 1 AND 30  THEN round(sum(fa_notaCreDeb_det.sc_total) - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO30, 
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) BETWEEN 31 AND 60 THEN round(sum(fa_notaCreDeb_det.sc_total) - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO60,
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) BETWEEN 61 AND 90 THEN round(sum(fa_notaCreDeb_det.sc_total) - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO90,
				  CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha,@FechaCorte) >= 91 THEN round(sum(fa_notaCreDeb_det.sc_total) - isnull(COBRO.dc_ValorPago,0),2) ELSE 0 END AS VENCIDO90MAS
FROM     fa_notaCreDeb INNER JOIN
                  ...
                  fa_notaCreDeb.IdNota = fa_notaCreDeb_det.IdNota left join
				  (
				  select d.IdEmpresa, d.IdSucursal, d.IdBodega_Cbte, d.IdCbte_vta_nota, d.dc_TipoDocumento, SUM(D.dc_ValorPago)dc_ValorPago
				  from cxc_cobro_det as d inner join cxc_cobro as c
				  on c.IdEmpresa = d.IdEmpresa and c.IdSucursal = d.IdSucursal and c.IdCobro = d.IdCobro
