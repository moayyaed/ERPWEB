...
SELECT fa_factura.IdEmpresa, fa_factura.IdSucursal, fa_factura.IdBodega, fa_factura.IdCbteVta, fa_factura.vt_serie1 + '-' + fa_factura.vt_serie2 + '-' + fa_factura.vt_NumFactura AS NumDocumento, fa_factura.vt_fecha, fa_factura.vt_fech_venc, 
                  tb_sucursal.Su_Descripcion, fa_factura.IdCliente, tb_persona.pe_nombreCompleto, tb_persona.pe_cedulaRuc, fa_factura.IdVendedor, fa_Vendedor.Ve_Vendedor, fa_factura_resumen.Total, CASE WHEN DATEDIFF(DAY,fa_factura.vt_fech_venc,@FechaFin) < 0 THEN 0 ELSE DATEDIFF(DAY,fa_factura.vt_fech_venc,@FechaFin) END DiasVencido,
...
FROM     fa_factura INNER JOIN
				  ...
				  where d.IdEmpresa = @IdEmpresa and c.cr_fecha <= @FechaFin and c.cr_estado = 'A'
...
and fa_factura.vt_fecha between @FechaIni and @FechaFin
UNION ALL
...
AS NumDocumento, fa_notaCreDeb.no_fecha, fa_notaCreDeb.no_fecha_venc, 
                  tb_sucursal.Su_Descripcion, fa_notaCreDeb.IdCliente, tb_persona.pe_nombreCompleto, tb_persona.pe_cedulaRuc, fa_notaCreDeb.IdVendedor, fa_Vendedor.Ve_Vendedor, D.Total, CASE WHEN DATEDIFF(DAY,fa_notaCreDeb.no_fecha_venc,@FechaFin) < 0 THEN 0 ELSE DATEDIFF(DAY,fa_notaCreDeb.no_fecha_venc,@FechaFin) END DiasVencido,
...
and fa_notaCreDeb.no_fecha between @FechaIni and @FechaFin
