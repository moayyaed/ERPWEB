--EXEC [web].[SPCONTA_007] 1,2019,'01/01/2019','31/12/2019','ADMIN',6,0,'',0
...
UPDATE web.ct_CONTA_007 SET DebitosSaldoInicial = ROUND(A.DebitosSaldoInicial,2), CreditosSaldoInicial = ROUND(a.CreditosSaldoInicial,2), SaldoInicial = ROUND(a.DebitosSaldoInicial - a.CreditosSaldoInicial,2)
FROM(
	select IdEmpresa, IdCtaCble, IdCentroCosto, sum(DebitosSaldoInicial) DebitosSaldoInicial, sum(CreditosSaldoInicial) CreditosSaldoInicial
	FROM(
			SELECT        ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCtaCble, isnull(ct_cbtecble_det.IdCentroCosto,'000') IdCentroCosto, ABS(SUM(ct_cbtecble_det.dc_Valor)) AS DebitosSaldoInicial, 0 CreditosSaldoInicial
			...
			GROUP BY ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCtaCble, ct_cbtecble_det.IdCentroCosto
			UNION ALL
			SELECT        ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCtaCble, isnull(ct_cbtecble_det.IdCentroCosto,'000') IdCentroCosto, ABS(SUM(ct_cbtecble_det.dc_Valor)) AS DebitosSaldoInicial, 0
			FROM            ct_cbtecble INNER JOIN
							ct_cbtecble_det ON ct_cbtecble.IdEmpresa = ct_cbtecble_det.IdEmpresa AND ct_cbtecble.IdTipoCbte = ct_cbtecble_det.IdTipoCbte AND ct_cbtecble.IdCbteCble = ct_cbtecble_det.IdCbteCble INNER JOIN
							ct_plancta ON ct_cbtecble_det.IdEmpresa = ct_plancta.IdEmpresa AND ct_cbtecble_det.IdCtaCble = ct_plancta.IdCtaCble INNER JOIN
							ct_grupocble ON ct_plancta.IdGrupoCble = ct_grupocble.IdGrupoCble LEFT JOIN
			...
			GROUP BY ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCtaCble, ct_cbtecble_det.IdCentroCosto
			UNION ALL
			SELECT        ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCtaCble, isnull(ct_cbtecble_det.IdCentroCosto,'000') IdCentroCosto, 0, ABS(SUM(ct_cbtecble_det.dc_Valor)) AS DebitosSaldoInicial
			FROM            ct_cbtecble INNER JOIN
							ct_cbtecble_det ON ct_cbtecble.IdEmpresa = ct_cbtecble_det.IdEmpresa AND ct_cbtecble.IdTipoCbte = ct_cbtecble_det.IdTipoCbte AND ct_cbtecble.IdCbteCble = ct_cbtecble_det.IdCbteCble INNER JOIN
							ct_plancta ON ct_cbtecble_det.IdEmpresa = ct_plancta.IdEmpresa AND ct_cbtecble_det.IdCtaCble = ct_plancta.IdCtaCble INNER JOIN
							ct_grupocble ON ct_plancta.IdGrupoCble = ct_grupocble.IdGrupoCble LEFT JOIN
			...
			UNION ALL
			--UTILIDAD
			SELECT        ct_cbtecble_det.IdEmpresa, @IdCtaCbleUtilidad, isnull(ct_cbtecble_det.IdCentroCosto,'000') IdCentroCosto, ABS(SUM(ct_cbtecble_det.dc_Valor)) AS DebitosSaldoInicial, 0
			FROM            ct_cbtecble INNER JOIN
							ct_cbtecble_det ON ct_cbtecble.IdEmpresa = ct_cbtecble_det.IdEmpresa AND ct_cbtecble.IdTipoCbte = ct_cbtecble_det.IdTipoCbte AND ct_cbtecble.IdCbteCble = ct_cbtecble_det.IdCbteCble INNER JOIN
							ct_plancta ON ct_cbtecble_det.IdEmpresa = ct_plancta.IdEmpresa AND ct_cbtecble_det.IdCtaCble = ct_plancta.IdCtaCble INNER JOIN
							ct_grupocble ON ct_plancta.IdGrupoCble = ct_grupocble.IdGrupoCble LEFT JOIN
			...
			WHERE        ct_cbtecble.IdEmpresa = @IdEmpresa and (ct_grupocble.gc_estado_financiero = 'ER') AND ct_cbtecble.cb_Fecha < @FechaIni and ct_cbtecble_det.dc_Valor > 0 AND @IdAnio = YEAR(ct_cbtecble.cb_Fecha)
			and f.IdUsuario = @IdUsuario
			GROUP BY ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCentroCosto
			UNION ALL
			SELECT        ct_cbtecble_det.IdEmpresa, @IdCtaCbleUtilidad, isnull(ct_cbtecble_det.IdCentroCosto,'000') IdCentroCosto, 0, ABS(SUM(ct_cbtecble_det.dc_Valor)) AS DebitosSaldoInicial
			FROM            ct_cbtecble INNER JOIN
							ct_cbtecble_det ON ct_cbtecble.IdEmpresa = ct_cbtecble_det.IdEmpresa AND ct_cbtecble.IdTipoCbte = ct_cbtecble_det.IdTipoCbte AND ct_cbtecble.IdCbteCble = ct_cbtecble_det.IdCbteCble INNER JOIN
							ct_plancta ON ct_cbtecble_det.IdEmpresa = ct_plancta.IdEmpresa AND ct_cbtecble_det.IdCtaCble = ct_plancta.IdCtaCble INNER JOIN
							ct_grupocble ON ct_plancta.IdGrupoCble = ct_grupocble.IdGrupoCble LEFT JOIN
	...
	GROUP BY IdEmpresa, IdCtaCble, IdCentroCosto
) A
where web.ct_CONTA_007.IdEmpresa = a.IdEmpresa and web.ct_CONTA_007.IdCtaCble = a.IdCtaCble and web.ct_CONTA_007.IdUsuario = @IdUsuario and web.ct_CONTA_007.IdCentroCosto = a.IdCentroCosto
END

BEGIN --ACTUALIZO DEBITOS Y CREDITOS
UPDATE web.ct_CONTA_007 SET Debitos = ROUND(A.DebitosSaldoInicial,2), Creditos = ROUND(a.CreditosSaldoInicial,2), SaldoDebitosCreditos = ROUND(a.DebitosSaldoInicial - a.CreditosSaldoInicial,2)
FROM(
...
where web.ct_CONTA_007.IdEmpresa = a.IdEmpresa and web.ct_CONTA_007.IdCtaCble = a.IdCtaCble and web.ct_CONTA_007.IdUsuario = @IdUsuario
END

BEGIN --ACTUALIZO SALDO FINAL
UPDATE web.ct_CONTA_007 SET SaldoDebitos = ROUND(A.DebitosSaldoInicial,2), SaldoCreditos = ROUND(a.CreditosSaldoInicial,2), SaldoFinal = ROUND(a.DebitosSaldoInicial - a.CreditosSaldoInicial,2)
FROM(
	select IdEmpresa, IdCtaCble, sum(DebitosSaldoInicial) DebitosSaldoInicial, sum(CreditosSaldoInicial) CreditosSaldoInicial
	FROM(
			SELECT        ct_cbtecble_det.IdEmpresa, ct_cbtecble_det.IdCtaCble, ABS(SUM(ct_cbtecble_det.dc_Valor)) AS DebitosSaldoInicial, 0 CreditosSaldoInicial
...
and IdEmpresa = @IdEmpresa
	...
	BEGIN
		...
		FROM            web.ct_CONTA_007
		where web.ct_CONTA_007.IdEmpresa = @IdEmpresa
		and web.ct_CONTA_007.IdUsuario = @IdUsuario
		GROUP BY IdEmpresa, IdCtaCblePadre
		
		) A where web.ct_CONTA_007.IdEmpresa = a.IdEmpresa
		and web.ct_CONTA_007.IdCtaCble = a.IdCtaCblePadre
		and web.ct_CONTA_007.IdUsuario = @IdUsuario
...
update web.ct_CONTA_007 Set SaldoFinal = round(SaldoFinal,2), SaldoFinalNaturaleza = round(SaldoFinalNaturaleza,2)
where IdUsuario = @IdUsuario
and IdEmpresa = @IdEmpresa

if(@MostrarAcumulado = 0)
BEGIN
	update web.ct_CONTA_007 SET SaldoFinal = SaldoDebitosCreditos, SaldoFinalNaturaleza = SaldoDebitosCreditosNaturaleza	
	where IdUsuario = @IdUsuario
	and IdEmpresa = @IdEmpresa

	IF(@MostrarSaldo0 = 0)
	BEGIN
		DELETE web.ct_CONTA_007
...
update web.ct_CONTA_007 set EsCuentaMovimiento = 1 
where IdUsuario = @IdUsuario
and IdEmpresa = @IdEmpresa
and not exists(
select * from web.ct_CONTA_007 as f
where f.IdEmpresa = web.ct_CONTA_007.IdEmpresa
and f.IdCtaCblePadre = web.ct_CONTA_007.IdCtaCble
and f.IdEmpresa = @IdEmpresa
and f.IdUsuario = @IdUsuario
)

UPDATE web.ct_CONTA_007 SET 
