...
SELECT OP.IdEmpresa, OP.IdOrdenPago, OP.IdTipo_op, OP.IdTipo_Persona, OP.IdPersona, OP.IdEntidad, OP.Fecha, OP.IdEstadoAprobacion, OP.IdFormaPago, OP.Estado, dbo.tb_persona.pe_nombreCompleto, CAN.Total_OP, 
                  ...
                  dbo.cp_orden_pago_estado_aprob.Descripcion, OP.IdSucursal, dbo.tb_sucursal.Su_Descripcion, CASE WHEN cp_orden_giro.co_factura IS NULL AND cp_nota_DebCre.cod_nota IS NOT NULL THEN cp_nota_DebCre.cod_nota
				  WHEN cp_orden_giro.co_factura IS NOT NULL AND cp_nota_DebCre.cod_nota IS NULL THEN cp_orden_giro.co_factura ELSE OP.IdTipo_op end as Referencia
