...
SELECT dbo.cp_orden_pago_cancelaciones.IdEmpresa, dbo.cp_orden_pago_cancelaciones.Idcancelacion, dbo.cp_orden_pago_cancelaciones.Secuencia, dbo.cp_orden_pago_cancelaciones.IdEmpresa_cxp, 
                  ...
                  THEN 'OP ' + CAST(cp_orden_pago_cancelaciones.IdOrdenPago_op AS VARCHAR(20)) WHEN cp_orden_giro.co_factura IS NULL AND cp_nota_DebCre.cod_nota IS NOT NULL 
                  THEN 'ND ' + cp_nota_DebCre.cod_nota ELSE 'FACT ' + CAST(CAST(cp_orden_giro.co_factura AS INT) AS VARCHAR(20)) END AS Referencia, CASE WHEN cp_orden_giro.co_factura IS NULL AND cp_nota_DebCre.cod_nota IS NULL 
				  ...
				  ISNULL(cp_orden_giro.co_FechaFactura, cp_nota_DebCre.cn_fecha) AS FechaCruce
