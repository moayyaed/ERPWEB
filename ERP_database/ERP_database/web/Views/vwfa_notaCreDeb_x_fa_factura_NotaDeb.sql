...
SELECT cruce.IdEmpresa_nt, cruce.IdSucursal_nt, cruce.IdBodega_nt, cruce.IdNota_nt, cruce.secuencia, cruce.vt_tipoDoc, cruce.IdEmpresa_fac_nd_doc_mod, cruce.IdSucursal_fac_nd_doc_mod, cruce.IdBodega_fac_nd_doc_mod, 
                  cruce.IdCbteVta_fac_nd_doc_mod, cruce.Valor_Aplicado, iif(cruce.vt_tipoDoc = 'FACT', factura.vt_NumFactura, debito.NumNotaImpresa) vt_NumFactura, iif(cruce.vt_tipoDoc = 'FACT', factura.vt_Observacion, debito.sc_observacion) 
                  vt_Observacion, iif(cruce.vt_tipoDoc = 'FACT', factura.CodCbteVta, debito.CodNota) CodDoc, iif(cruce.vt_tipoDoc = 'FACT', factura.vt_Subtotal, debito.sc_subtotal) vt_Subtotal, iif(cruce.vt_tipoDoc = 'FACT', factura.vt_iva, debito.sc_iva) 
                  vt_iva, iif(cruce.vt_tipoDoc = 'FACT', factura.vt_total, debito.sc_total) vt_total, isnull(cobro.ValorCobrado, 0) ValorCobrado, isnull(iif(cruce.vt_tipoDoc = 'FACT', factura.vt_total, debito.sc_total) - isnull(cobro.ValorCobrado, 0), 0) AS saldo, 
                  iif(cruce.vt_tipoDoc = 'FACT', factura.vt_total, debito.sc_total) - isnull(cobro.ValorCobrado, 0) + cruce.Valor_Aplicado AS saldo_sin_cobro, iif(cruce.vt_tipoDoc = 'FACT', factura.vt_fecha, debito.no_fecha) vt_fecha, 
                  iif(cruce.vt_tipoDoc = 'FACT', factura.vt_fech_venc, debito.no_fecha_venc) vt_fech_venc, iif(cruce.vt_tipoDoc = 'FACT', factura.IdCliente, debito.IdCliente) IdCliente, iif(cruce.vt_tipoDoc = 'FACT', factura.Nombres, debito.Nombres) 
                  pe_nombreCompleto,cruce.NumDocumento
FROM     fa_notaCreDeb_x_fa_factura_NotaDeb AS cruce LEFT JOIN
                      (SELECT fa_factura.IdEmpresa, fa_factura.IdSucursal, fa_factura.IdBodega, fa_factura.IdCbteVta, fa_factura.vt_tipoDoc, fa_factura.vt_tipoDoc + '-' + CAST(CAST(fa_factura.vt_NumFactura AS NUMERIC) AS VARCHAR(20)) 
                                         vt_NumFactura, fa_factura.vt_Observacion, fa_factura.CodCbteVta, SUM(fa_factura_det.vt_Subtotal) AS vt_Subtotal, SUM(fa_factura_det.vt_iva) AS vt_iva, SUM(fa_factura_det.vt_total) AS vt_total, fa_factura.vt_fecha, 
                                         fa_factura.vt_fech_venc, fa_factura.IdCliente, tb_persona.pe_nombreCompleto Nombres
                       ...
                       GROUP BY fa_factura.IdEmpresa, fa_factura.IdSucursal, fa_factura.IdBodega, fa_factura.IdCbteVta, fa_factura.vt_tipoDoc, fa_factura.vt_NumFactura, fa_factura.vt_Observacion, fa_factura.CodCbteVta, fa_factura.vt_fecha, 
                                         fa_factura.vt_fech_venc, fa_factura.IdCliente, tb_persona.pe_nombreCompleto) AS factura ON cruce.IdEmpresa_fac_nd_doc_mod = factura.IdEmpresa AND cruce.IdSucursal_fac_nd_doc_mod = factura.IdSucursal AND 
                  cruce.IdBodega_fac_nd_doc_mod = factura.IdBodega AND cruce.IdCbteVta_fac_nd_doc_mod = factura.IdCbteVta AND cruce.vt_tipoDoc = factura.vt_tipoDoc LEFT JOIN
                  ...
                  cruce.IdEmpresa_fac_nd_doc_mod = debito.IdEmpresa AND cruce.IdSucursal_fac_nd_doc_mod = debito.IdSucursal AND cruce.IdBodega_fac_nd_doc_mod = debito.IdBodega AND cruce.IdCbteVta_fac_nd_doc_mod = debito.IdNota AND 
                  cruce.vt_tipoDoc = debito.CodDocumentoTipo LEFT OUTER JOIN
