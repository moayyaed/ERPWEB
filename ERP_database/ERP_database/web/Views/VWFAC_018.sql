...
SELECT nc.IdEmpresa, nc.IdSucursal, nc.IdBodega, nc.IdNota, s.Su_Descripcion, nc.no_fecha, nc.IdCliente, t.IdTipoNota, t.No_Descripcion, 
                  ...
                  AS ValorAplicado, 
                  (CASE WHEN ncx.vt_tipoDoc = 'FACT' THEN f.vt_serie1 + '-' + f.vt_serie2 + '-' + f.vt_NumFactura ELSE CASE WHEN nd.NaturalezaNota = 'SRI' THEN nd.Serie1 + '-' + nd.Serie2 + '-' + nd.NumNota_Impresa ELSE ISNULL(ND.CODNOTA, 
                  CAST(nd.IdNota AS varchar(20))) END END) AS NumDocumentoAplica, ncx.NumDocumento AS NumDocumentoReemplazo, 
				  Case when ISNULL(ncx.secuencia, 1) = 1 then ncd.SubtotalSinIVAConDscto else 0 end AS Subtotal0,
				  Case when ISNULL(ncx.secuencia, 1) = 1 then ncd.SubtotalIVAConDscto else 0 end AS SubtotalIVA, 
				  Case when ISNULL(ncx.secuencia, 1) = 1 then ncd.ValorIVA else 0 end AS ValorIVA, 
				  Case when ISNULL(ncx.secuencia, 1) = 1 then ncd.Total else 0 end AS Total,                   
				  nc.Estado, CASE WHEN nc.Estado = 'I' THEN 'ANULADAS' ELSE 'ACTIVAS' END AS NomEstado, CASE WHEN nc.Estado = 'I' THEN 2 ELSE 1 END AS Orden, nc.CreDeb, nc.NaturalezaNota
FROM     dbo.fa_notaCreDeb AS nc LEFT OUTER JOIN
                  ...
                  dbo.fa_notaCreDeb AS nd ON ncx.IdEmpresa_fac_nd_doc_mod = nd.IdEmpresa AND ncx.IdSucursal_fac_nd_doc_mod = nd.IdSucursal AND ncx.IdBodega_fac_nd_doc_mod = nd.IdBodega AND 
                  ncx.IdCbteVta_fac_nd_doc_mod = nd.IdNota AND ncx.vt_tipoDoc = nd.CodDocumentoTipo LEFT OUTER JOIN
