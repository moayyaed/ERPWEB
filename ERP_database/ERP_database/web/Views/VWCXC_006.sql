...
UNION ALL
SELECT dbo.cxc_cobro_det.IdEmpresa, dbo.cxc_cobro_det.IdSucursal, dbo.cxc_cobro_det.IdCobro, dbo.cxc_cobro_det.secuencial, dbo.fa_notaCreDeb.CodDocumentoTipo, dbo.fa_notaCreDeb.IdBodega, dbo.fa_notaCreDeb.IdNota, dbo.fa_Vendedor.Ve_Vendedor, 
                  dbo.fa_notaCreDeb.no_fecha, null, null, dbo.fa_notaCreDeb.CodNota, dbo.fa_notaCreDeb.IdCliente, dbo.tb_persona.pe_nombreCompleto, dbo.fa_notaCreDeb.IdVendedor, 
                  dbo.fa_notaCreDeb.Serie1 + '-' + dbo.fa_notaCreDeb.Serie2 + '-' + dbo.fa_notaCreDeb.NumNota_Impresa AS vt_NumFactura, dbo.fa_notaCreDeb.no_fecha_venc, dbo.cxc_cobro.cr_fecha, 
                  CASE WHEN fa_notaCreDeb.no_fecha_venc > cxc_cobro.cr_fecha THEN 0 ELSE DATEDIFF(DAY, fa_notaCreDeb.no_fecha_venc, cxc_cobro.cr_fecha) END AS DiasAtraso, dbo.cxc_cobro_det.dc_ValorPago AS ValorPago, 
                  CASE WHEN fa_notaCreDeb_resumen.ValorIVA > 0 THEN round(cxc_cobro_det.dc_ValorPago / 1.12, 2) ELSE cxc_cobro_det.dc_ValorPago END AS BaseComision, dbo.cxc_cobro_det.IdCobro_tipo
FROM     dbo.cxc_cobro_det INNER JOIN
                  dbo.cxc_cobro ON dbo.cxc_cobro_det.IdEmpresa = dbo.cxc_cobro.IdEmpresa AND dbo.cxc_cobro_det.IdSucursal = dbo.cxc_cobro.IdSucursal AND dbo.cxc_cobro_det.IdCobro = dbo.cxc_cobro.IdCobro INNER JOIN
                  dbo.fa_cliente INNER JOIN
                  ...
                  dbo.fa_notaCreDeb_resumen ON dbo.fa_notaCreDeb.IdEmpresa = dbo.fa_notaCreDeb_resumen.IdEmpresa AND dbo.fa_notaCreDeb.IdSucursal = dbo.fa_notaCreDeb_resumen.IdSucursal AND dbo.fa_notaCreDeb.IdBodega = dbo.fa_notaCreDeb_resumen.IdBodega AND 
                  dbo.fa_notaCreDeb.IdNota = dbo.fa_notaCreDeb_resumen.IdNota ON dbo.cxc_cobro_det.IdEmpresa = dbo.fa_notaCreDeb.IdEmpresa AND dbo.cxc_cobro_det.IdSucursal = dbo.fa_notaCreDeb.IdSucursal AND 
