...
SELECT dbo.fa_notaCreDeb_det.IdEmpresa, dbo.fa_notaCreDeb_det.IdSucursal, dbo.fa_notaCreDeb_det.IdBodega, dbo.fa_notaCreDeb_det.IdNota, dbo.fa_notaCreDeb_det.Secuencia, dbo.fa_notaCreDeb.Serie1, dbo.fa_notaCreDeb.Serie2, 
                  dbo.fa_notaCreDeb.NumNota_Impresa, dbo.tb_persona.pe_nombreCompleto, dbo.tb_persona.pe_cedulaRuc, dbo.fa_notaCreDeb.no_fecha, ISNULL(cr_fa.vt_fecha, cr_nd.no_fecha) AS FechaDocumentoAplica, 
                  ...
                  THEN CASE WHEN fa_notaCreDeb.NaturalezaNota = 'SRI' THEN fa_notaCreDeb.Serie1 + '-' + fa_notaCreDeb.Serie2 + '-' + fa_notaCreDeb.NumNota_Impresa ELSE ISNULL(fa_notaCreDeb.CodNota, 
                  CAST(fa_notaCreDeb.IdNota AS varchar(20))) END ELSE cr_fa.vt_serie1 + '-' + cr_fa.vt_serie2 + '-' + cr_fa.vt_NumFactura END AS DocumentoAplicado, dbo.fa_notaCreDeb.NumAutorizacion, dbo.fa_notaCreDeb.Fecha_Autorizacion, 
                  dbo.fa_notaCreDeb.CreDeb, dbo.fa_notaCreDeb.CodDocumentoTipo
